...
export default function AuthModal({ open, onClose }: AuthModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const ok = await loginMutation.mutateAsync({ username: username.trim(), password });
      if (ok) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const ok = await registerMutation.mutateAsync({ username: username.trim(), password });
      if (ok) {
          ...
          <div className="p-7 pt-5">
            <DialogHeader className="mb-5">
              <div className="flex justify-center mb-3">
                ...
                >
                  {tab === 'login' ? (
                    <LogIn size={28} style={{ color: 'oklch(0.75 0.20 295)' }} />
                  ) : (
                    <UserPlus size={28} style={{ color: 'oklch(0.75 0.20 295)' }} />
                  )}
                </div>
              </div>
              <DialogTitle
              ...
              >
                {tab === 'login' ? 'Welcome back, legend!' : 'Join the ZoritLegends community'}
              </DialogDescription>
            </DialogHeader>
              ...
              {(['login', 'register'] as Tab[]).map((t) => (
                <button
                  ...
                  style={{
                    ...
                    background: tab === t ? 'oklch(0.55 0.25 295)' : 'oklch(0.10 0.05 295)',
                    color: tab === t ? 'oklch(0.99 0 0)' : 'oklch(0.60 0.10 295)',
                    border: 'none',
                    cursor: 'pointer',
                  }}
                >
            ...
            <form onSubmit={tab === 'login' ? handleLogin : handleRegister} className="space-y-3">
              ...
              <button
                ...
                style={{
                  background: isLoading ? 'oklch(0.40 0.15 295)' : 'oklch(0.55 0.25 295)',
              ...
              {tab === 'login' ? (
                <>
                  ...
                  <button
                    ...
                    style={{ color: 'oklch(0.72 0.18 295)', fontFamily: '"VT323", monospace', fontSize: '0.9rem' }}
                  >
                    Register here
                  </button>
                </>
...
function PixelInput({ icon, label, type, value, onChange, placeholder, disabled }: PixelInputProps) {
  ...
  return (
    <div>
      <p
        ...
        style={{ fontSize: '0.45rem', color: 'oklch(0.55 0.15 295)', letterSpacing: '0.06em' }}
      >
